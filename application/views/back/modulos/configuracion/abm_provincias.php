        ...
        <div class="col-md-12">
            <div class="box">
                <div class="box-header">
                    <button class="btn btn_primary btn-primary" onCLick='$("#modal_agregar").modal("show");'><i class='fa fa-plus'></i> Agregar Provincia</button>
                </div><!-- /.box-header -->
                <div class="box-body">
                  <table id="listado" class="table table-bordered table-hover">
                    <thead>
                      <tr>
                        <th>Provincia</th>
                        <th style="width: 100px;">Localidades</th>
                    ...
                    <tbody>
                      <?php foreach ($provincias as $value)
                      {
                        ...
                        <tr>
                            <td>
                              <span id='provincia_".$value["id"]."'>".$value["provincia"]."</span>
                              </td>
                            <td>
                              <a href='".base_url()."index.php/Localidades/abm_localidades/".$value["id"]."' class='btn btn-sm btn-primary'>
                                <i class='fa fa-eye'></i> 
                                Localidades
                              </a>
                            </td>
                            <td> 
                                <a href='#' class='btn btn-sm btn-primary' data-toggle='tooltip' title='' data-original-title='Editar' onCLick='abrir_modal_editar(".$value["id"].")'><i class='fa fa-edit'></i></a>&nbsp;
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Agregar una provincia:</h4>
            </div>
            <div class="modal-body">
                <form action="<?=base_url()?>index.php/Provincia/agregar_provincia" method="post" id="formulario_agregar">
                <div class="col-md-12">
                    <div class="form-group">
                        <label for="provincia_agregar">Provincia*</label>
                        <input class="form-control" id="provincia_agregar" name="provincia_agregar" value="" type="text">
                    </div>
                </div>
...
<div class="modal modal-default" id="modal_editar">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?=base_url()?>index.php/Provincia/editar_provincia" method="post" id="formulario_editar">
                <div class="col-md-12">
                    <input type='text' id='id_editar' name="id_editar" hidden="">
                    <div class="form-group">
                        <label for="provincia_editar">Provincia*</label>
                        <input class="form-control" id="provincia_editar" name="provincia_editar" value="" type="text">
                    </div>
                </div>
...
<div class="modal modal-default" id="modal_eliminar">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?=base_url()?>index.php/Provincia/eliminar_provincia" method="post" id="formulario_eliminar">
                    <input type='text' id='id_eliminar' name="id_eliminar" hidden="">
                    <div class="col-md-12">
                        <p style="font-size: 17px;font-weight: bold;text-align: center;">¿Seguro desea eliminar la provincia seleccionada con sus localidades?</p>
                    </div>
...
<script>
    ...
    {
      $("#provincia_agregar").val("");
      $("#modal_agregar").modal("show");
    }

    function agregar()
    {
        if(validar_vacio("provincia_agregar"))
        {
            var fd = new FormData($("#formulario_agregar")[0]);

            $.ajax({
                url: "<?php echo base_url()?>index.php/Provincia/agregar_provincia",
                type: "POST",
                data:fd,
                processData: false,
                contentType: false,
                success: function(data)
                {
                  ...
                  if(data["respuesta"] == true)
                  {
                    ...
                    var html_button1=
                    "<a href='<?php echo base_url() ?>index.php/Localidades/abm_localidades/"+row["id"]+"' class='btn btn-sm btn-primary'><i class='fa fa-eye'></i>Localidades</a>";

                    var html_button2="<a href='#' class='btn btn-sm btn-primary' data-toggle='tooltip' title='' data-original-title='Editar' onCLick='abrir_modal_editar("+row["id"]+")'><i class='fa fa-edit'></i></a>&nbsp;\n\
                                \n\
                    ...
                    var provincia = "<span id='provincia_"+row["id"]+"'>"+row["provincia"]+"</span>";

                    mi_datatable.row.add([provincia,html_button1,html_button2]).draw();
                  }
                  else
                  {
                ...
                error: function(event){
                  alert("No se ha podido agregar, compruebe que la provincia no se encuentre agregada");
                },
            });    
        }
    }
    ...
    {
      $.ajax({
          url: "<?php echo base_url()?>index.php/Provincia/get_provincia",
          type: "POST",
          data:{id:id},
          success: function(data)
          {
            ...
            if(data["respuesta"])
            {
              ...
              $("#provincia_editar").val(row["provincia"]);
    ...
    function editar()
    {
        if(validar_vacio("provincia_editar") && validar_vacio("id_editar"))
        {
            var fd = new FormData($("#formulario_editar")[0]);

            $.ajax({
                url: "<?php echo base_url()?>index.php/Provincia/editar_provincia",
                type: "POST",
                data:fd,
                processData: false,
                contentType: false,
                success: function(data)
                {
                  ...
                  if(data["respuesta"])
                  {
                    ...
                    var object  = $("#provincia_"+row["id"]).parent("td");
                    var cell = mi_datatable.cell(object);
                    cell.data(row["provincia"]).draw();
                ...
                error: function(event){
                  alert("No se ha podido editar, compruebe que la provincia no se encuentre agregada");
                },
            });    
        }
    }
    ...
    function eliminar()
    {
      ...
      $.ajax({
          url: "<?php echo base_url()?>index.php/Provincia/eliminar_provincia",
          type: "POST",
          data:fd,
          processData: false,
          contentType: false,
          success: function(data)
          {
            ...
            if(data["respuesta"])
            {
              ...
              var tr =$("#provincia_"+id).parent("td");
              tr = tr.parent("tr");
